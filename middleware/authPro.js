...
const authPro = async(req,res,next)=>{                                 // se ejecuta desp de que se ejecute la funcion auth corroborando ANTES QUE NADA que haya token          
    try {                                                
    ...
    if(user.role === 0 || user.role === 2){
        return res.status(400).json({
            success:false,
            message: "Access denied, you are not a Pro user"
        })
    }
