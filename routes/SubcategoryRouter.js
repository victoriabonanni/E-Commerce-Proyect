...
//   const { id } = req.params;
//   try {
//     let subcategory = await Subcategory.findById(id); // "espera" busca dentro de la colección creada Category a través del metodo ID y me va a devolver todo el objeto con ese ID.
//     return res.status(200).json({
...
SubcategoryRouter.put("/subcategory/:id", auth, authAdmin, async (req, res) => {
  ...
  const { title, description } = req.body; // paso por el body el valor que voy a querer cambiar
  try {
