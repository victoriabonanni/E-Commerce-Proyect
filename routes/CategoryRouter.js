...
CategoryRouter.post("/category", auth, authAdmin, async (req, res) => {                    // añado objetos a mi base de datos respetando ciertas condiciones
  ...
  const { title } = req.body;          // paso tambien la subcategory por el body, pero no es obligatorio
  try {
    if (!title) {                             // condiciones para crear mis objetos en la DB - son condiciones para nosotros, no tiene nada q ver con el cliente, esos mensajes no son alertas q ve el cliente
...
CategoryRouter.get("/category/:id", async (req, res) => {                 // Devuelve un objeto en concreto a través del ID de la DB
  const {id} = req.params
  try {
    let category = await Category.findById(id)                            // "espera" busca dentro de la colección creada Category a través del metodo ID y me va a devolver todo el objeto con ese ID. 
...
CategoryRouter.put("/category/:id", auth, authAdmin, async (req,res)=>{                   // Modificar un objeto en concreto
  ...
  const {title} = req.body                                              // paso por el body el valor que voy a querer cambiar
  try {
